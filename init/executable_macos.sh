...
# ~/.macos — from https://mths.be/macos
...
cd ~/Library/Fonts && {
  if ! [ -f MesloLGS%20NF%20Regular.ttf ]; then
...
grep -qxF 'pam_tid.so' /etc/pam.d/sudo || echo "auth       sufficient     pam_tid.so" | cat - /etc/pam.d/sudo | sudo tee /etc/pam.d/sudo
